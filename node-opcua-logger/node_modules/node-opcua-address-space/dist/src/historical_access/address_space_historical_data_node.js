    ...
    push(newDataValue) {
        ...
        if (sourceTime.getTime() <= this.lastDate.getTime()) {
            if (!(sourceTime.getTime() === this.lastDate.getTime() && sourcePicoSeconds > this.lastDatePicoSeconds)) {
                console.log(chalk_1.default.red("Warning date not increasing "), newDataValue.toString(), " last known date = ", this.lastDate);
            }
        }
...
function _historyReadRaw(context, historyReadRawModifiedDetails, indexRange, dataEncoding, continuationPoint, callback) {
    ...
    if (node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.endTime)) {
        ...
        if (node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.startTime)) {
            ...
            const result = new node_opcua_service_history_1.HistoryReadResult({
                statusCode: node_opcua_status_code_1.StatusCodes.BadHistoryOperationInvalid // should be an error
            });
            return callback(null, result);
        }
    }
