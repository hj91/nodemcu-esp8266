...
function visitor_IMPORT(n) {
  // eslint-disable-line camelcase
  var ni = n.type === 'ImportDeclaration';
  if (!ni) return null;
  var s = n.specifiers;
...
function reconstructSpecifiers(specs) {
  ...
  try {
    for (var _iterator = (0, _getIterator3.default)(specs), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  try {
    for (var _iterator2 = (0, _getIterator3.default)(specs), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var _spec = _step2.value;

      if (_spec.type === 'ImportSpecifier') {
        if (_spec.local.name === _spec.imported.name) {
          nonDefaults.push(_spec.local.name);
        } else {
          nonDefaults.push(_spec.imported.name + ' as ' + _spec.local.name);
        }
      }
    }
  } catch (err) {
